    ...
    while (true) {
        ...
        if (gpio_get(BUTTON) == 0 && !led_active) {
            ...
            if (gpio_get(BUTTON) == 0 && gpio_get(LED_RED) == 0 && gpio_get(LED_BLUE) == 0 && gpio_get(LED_GREEN) == 0) {
                ...
                add_alarm_in_ms(4000, turn_off_callback2, NULL, false); // Desliga o LED vermelho em 4s
                add_alarm_in_ms(5000, turn_off_callback3, NULL, false); // Desliga o LED verde em 5s
            }
        }
